...
import { getAvailableOrdersForRider } from '@/app/lib/delivery';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const orders = await getAvailableOrdersForRider(user.id);

    return NextResponse.json({ success: true, data: orders }, { status: 200 });
  } catch (error: any) {
    console.error('[API Delivery Available GET] Error:', error);
